...
class TestLeafNode(unittest.TestCase):
    def test_to_html(self):
        node = LeafNode("a", "This is a paragraph of text.", props={"href": "https://www.google.com"})
        result = node.to_html()
        self.assertEqual(result, '<a href="https://www.google.com">This is a paragraph of text.</a>')

    def test_no_tag(self):
        node = LeafNode(None, "This is a paragraph of text.", props={"href": "https://www.google.com"})
        result = node.to_html()
        ...
        node = ParentNode("div",
            ...
            ParentNode(
                "section",
                    ...
                    LeafNode("a", "Link text", props={"href": "https://example.com"})
                    ]
                )
            ]
        )
        result = node.to_html()
        self.assertEqual(result, '<div><h1>Main Title</h1><section><p>Paragraph in section</p><a href="https://example.com">Link text</a></section></div>')

    def empty_children(self):
        with self.assertRaises(ValueError) as context:
            node = ParentNode("div", [])
        self.assertTrue("ParentNode must contain at least one child argument" in str(context.exception))
        ...
        node = ParentNode(
            ...
            [
                ...
                ParentNode(
                    "li",
                    [
                        LeafNode("p", "Paragraph in item 2")
                    ]
                )
            ]
        )
                    ...
                    [
                        ParentNode(
                            ...
                            [
                                ...
                                LeafNode("a", "Click here", {"href": "https://example.com/item1"})
                            ]
                        ),
                        ParentNode(
                            "li",
                            [
                                ...
                                ParentNode(
                                    "div",
                                    [
                                        LeafNode("span", "More details here.")
                                    ]
                                )
                            ]
                        )
